...
async def get_current_epoch(
    ...
    try:
        [current_epoch_data] = await request.app.state.anchor_rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.currentEpoch(settings.data_market_id)],
        )
        current_epoch = {
...
async def get_epoch_info(
    ...
    try:
        [epoch_info_data] = await request.app.state.anchor_rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.getEpochInfo(settings.data_market_id, epoch_id)],
        )
        epoch_info = {
    ...
    try:
        ...
        while not epoch_finalized and epoch_id >= 0:
            ...
            [epoch_finalized_contract] = await request.app.state.anchor_rpc_helper.web3_call(
                [request.app.state.protocol_state_contract.functions.getSnapshotStatus(settings.data_market_id, project_id, epoch_id)],
            )
        ...
        [epoch_info_data] = await request.app.state.anchor_rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.getEpochInfo(settings.data_market_id, project_last_finalized_epoch)],
        )
        epoch_info = {
...
async def get_task_status_post(
    ...
    try:
        ...
        [last_finalized_epoch] = await request.app.state.anchor_rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.lastFinalizedSnapshot(settings.data_market_id, project_id)],
        )
