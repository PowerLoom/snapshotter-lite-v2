...
async def get_current_epoch(
    ...
    try:
        [current_epoch_data] = await request.app.state.anchor_rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.currentEpoch(settings.data_market)],
        )
        current_epoch = {
...
async def get_epoch_info(
    ...
    try:
        [epoch_info_data] = await request.app.state.anchor_rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.epochInfo(settings.data_market, epoch_id)],
        )
        epoch_info = {
...
async def get_project_last_finalized_epoch_info(
    ...
    try:
        ...
        [epoch_info_data] = await request.app.state.anchor_rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.epochInfo(settings.data_market, project_last_finalized_epoch)],
        )
        epoch_info = {
...
async def get_task_status_post(
    ...
    try:
        ...
        [last_finalized_epoch] = await request.app.state.anchor_rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.lastFinalizedSnapshot(settings.data_market, project_id)],
        )
